...
use crate::{novenybuilder, novenyfield, request};
    ...
    fn getfield(&self, field: &str)-> Option<String>{
        if field == "nemszeret"{
            match &self.nemszeret {
                None => return None,
                Some(nemszeret) => return Some(nemszeret.iter().map(|n|n.to_string()).collect::<Vec<String>>().join(", ").to_string())
            }
        }
...
fn makerequest(builder: RequestBuilder) -> Output{
    ...
    if method == "post"{
        ...
        params.push("POST".to_string());
        params.push(format!("{}",url+"/"+&builder.table.unwrap()));
        params.push("-H".to_string());
        params.push("Content-Type: application/json".to_string());
        params.push("-H".to_string());
        params.push("Prefer: return=minimal".to_string());
        params.push("-d".to_string());
        params.push(builder.json.unwrap());
    } else {
        params.push(format!("{}",url+"/"+&builder.table.unwrap()));
